...
void SquadCellView::getCellDimension(MyGUI::Widget* _sender, MyGUI::IntCoord& _coord, bool _drop)
{
	if (_drop)
		_coord.set(0, 0,180, 40);
	else
		_coord.set(0, 0,180, 40);
}

SquadCellView::SquadCellView(MyGUI::Widget* _parent) :
wraps::BaseCellView<SquadItemData*>("SquadCellView.layout", _parent)
{
...
void SquadCellView::update(const MyGUI::IBDrawItemInfo& _info, SquadItemData* _data)
{
	if (_info.update)
	{
		if (!_data->isEmpty())
		...
		mTextBack->setCaption(_data->getLevel());
